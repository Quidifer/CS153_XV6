...
fetchint(uint addr, int *ip)
{
  ...
  // if(addr >= curproc->sz || addr+4 > curproc->sz) //old 
  //   return -1;
  if(addr >= KERNBASE-1 || addr+4 > KERNBASE-1) // new for lab 3
    return -1;  
  *ip = *(int*)(addr);
...
fetchstr(uint addr, char **pp)
{
  ...
  // if(addr >= curproc->sz) // old
  //   return -1;
  if(addr >= KERNBASE-1) // new for lab 3
    return -1;
  *pp = (char*)addr;
  
  // ep = (char*)curproc->sz; // old
  ep = (char*)KERNBASE-1; // new for lab 3
  for(s = *pp; s < ep; s++){
    if(*s == 0)
      return s - *pp;
  }
...
argptr(int n, char **pp, int size)
{
  ...
  //   return -1;
  if(size < 0 || (uint)i >= KERNBASE-1 || (uint)i+size > KERNBASE-1) // new for lab 3
    return -1;
  *pp = (char*)i;
  return 0;
}
